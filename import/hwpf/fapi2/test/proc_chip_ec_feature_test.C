...
/*                                                                        */
/* $Source: import/hwpf/fapi2/test/proc_chip_ec_feature_test.C $          */
/*                                                                        */
...
{
    // procedure constants
    const std::string PROCEDURE = "proc_ec_feature_test";
...
int main(int argc, char* argv[])
{
    ...
    target.chipUnitType  = ECMD_TARGET_FIELD_UNUSED; // "mcbist";   // "mcs";     // "mba";
    ...
    /*
        ...
        //   // We are not dealing with chipunits here so let the Dll know we
        //   want to know everything above that
    ...
    */
    ...
    {
        if (1) //!ecmdGetGlobalVar(ECMD_GLOBALVAR_QUIETMODE))
        {
            snprintf(outstr, sizeof(outstr), "Going to call %s on proc k%d:n%d:s%d:p%02d\n", PROCEDURE.c_str(), target.cage,
                     target.node, target.slot, target.pos);
            ecmdOutput(outstr);
        }
        ...
        FAPI_EXEC_HWP(l_rc, proc_chip_ec_feature, fapi_target);

        rc = (uint64_t) l_rc;

        if (rc)
        {
            snprintf(outstr, sizeof(outstr), "ERROR: %s FAPI call exited with return code = %s 0x%08x \n", PROCEDURE.c_str(),
