...
/*                                                                        */
/* $Source: import/chips/p10/common/occ/commonlib/kernel.h $              */
/*                                                                        */
...
//Types
#define KERN_SEMAPHORE              uint64_t
#define KERN_SEMAPHORE_COUNT        uint32_t
#define KERN_DEQUE                  uint64_t
#define KERN_THREAD                 void   // iota does not support threads
#define KERN_THREAD_PRIORITY        PkThreadPriority
#define KERN_THREAD_STATE           PkThreadState
#define KERN_THREAD_ROUTINE         PkThreadRoutine
#define KERN_THREAD_FLAGS           PkThreadFlags
...
#define KERN_SEMAPHORE_CREATE(semaphore, initial_count, max_count)
#define KERN_SEMAPHORE_STATIC_CREATE(sem, initial_count, max_count)
#define KERN_SEMAPHORE_INITIALIZATION(_initial_count, _max_count) 0
...
// IOTA does not support message queues
#define KERN_DEQUE_SENTINEL_CREATE(deque) *deque = 0
#define KERN_DEQUE_SENTINEL_STATIC_CREATE(deque) deque = 0
#define KERN_DEQUE_SENTINEL_INIT(dq_addr) dq_addr = 0

#define KERN_DEQUE_ELEMENT_CREATE(element) *element = 0
#define KERN_DEQUE_ELEMENT_STATIC_CREATE(deque) deque = 0
#define KERN_DEQUE_ELEMENT_INIT() 0

#define KERN_DEQUE_IS_EMPTY(deque) 0
#define KERN_DEQUE_IS_QUEUED(element) 0
#define KERN_DEQUE_PUSH_BACK(deque, element)
#define KERN_DEQUE_PUSH_FRONT(deque, element)
#define KERN_DEQUE_POP_FRONT(deque)
#define KERN_DEQUE_DELETE(element)

#define KERN_IRQ_HANDLER(f) void f()
#define KERN_IRQ_SETUP(irq, polarity, trigger) \
    ppe_irq_setup(irq, polarity, trigger)

// IPC handler should be set in IOTA_TASK_TABLE
#define KERN_IRQ_HANDLER_SET(irq, handler, arg, priority) 0
#define KERN_IRQ_ENABLE(irq) \
    ppe_irq_enable(irq)
#define KERN_IRQ_DISABLE(irq) \
    ppe_irq_disable(irq)
#define KERN_IRQ_STATUS_CLEAR(irq) \
    ppe_irq_status_clear(irq)
#define KERN_IRQ_STATUS_SET(irq, value) \
    ppe_irq_status_set(irq, value)
#define KERN_IRQ_FAST2FULL(fast_handler, full_handler) \
    PK_IRQ_FAST2FULL(fast_handler, full_handler)

#define KERN_CRITICAL_SECTION_ENTER(priority, pctx) \
    ppe_critical_section_enter(pctx)
#define KERN_CRITICAL_SECTION_EXIT(pctx) \
    ppe_critical_section_exit(pctx)
#define KERN_CONTEXT_CRITICAL_INTERRUPT() \
    (0)
...
#define KERN_STATIC_ASSERT(cond) \
    extern uint8_t _static_assert[(cond) ? 1 : -1]  __attribute__ ((unused))
