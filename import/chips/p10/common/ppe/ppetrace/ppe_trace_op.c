                  ...
                  uint32_t* i_payload)
{
    ...
    cur_offset = g_pk_op_trace_buf.state.offset;
    ...
    if(mark.fields.timestamp)
    {
        ...
        g_pk_op_trace_buf.state.word64 = state.word64;
    }
    else
    {
        //update the cb state offset
        g_pk_op_trace_buf.state.offset = state.offset;
    }

    footer_offset = state.offset - sizeof(footer);

    //calculate the address of the footer
    ptr32 = (uint32_t*)&g_pk_op_trace_buf.cb[footer_offset & PK_OP_TRACE_CB_MASK];
    ...
    *ptr32 = footer.word32;
    ...
    {
        ptr32 = (uint32_t*)&g_pk_op_trace_buf.cb[(cur_offset + i) & PK_OP_TRACE_CB_MASK];
        *ptr32 = *i_payload++;
    }
...
#if (PK_TRACE_SUPPORT)
void ppe_trace_pk(uint32_t i_mark,
                  uint32_t* i_payload)
{
    PkTraceTime         footer;
    uint32_t            footer2 = 0;
...
{
    ppe_trace_pk(i_mark, NULL);
}

void ppe_trace_pk2(uint32_t i_mark, uint32_t parm1)
{
    ppe_trace_pk(i_mark, &parm1);
}
