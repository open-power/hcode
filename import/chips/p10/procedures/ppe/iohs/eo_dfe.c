...
int32_t rx_eo_dfe_check_dac_limits(t_gcr_addr* i_tgt, const uint32_t i_dac_addr, const int32_t i_new_val)
{
    const int32_t LOWER_LIMIT = -255;
    const int32_t UPPER_LIMIT =  255;
        ...
        int32_t* io_clk_adj)
{
    ...
    int32_t       l_ap          = eo_dfe_round(i_ap1 + i_ap0) >> 1; // Average AP1 & AP0 = AP
...
static inline void rx_eo_dfe_read_loff(t_gcr_addr* i_tgt, int16_t o_loff_array[DAC_SIZE])
{
    ...
    {
        o_loff_array[l_index] = SignedMagToInt(get_ptr(i_tgt, l_dac_addr, DAC_STARTBIT, DAC_ENDBIT), DAC_WIDTH);
    }
 ...
 * @return uint32_t. pass_code if success, else error code.
 */
uint32_t rx_eo_dfe_fast_servo(t_gcr_addr* i_tgt, int32_t i_loff[4], int32_t o_hvals[3], int32_t* io_clk_adj)
{
    ...
    o_hvals[0] = eo_dfe_round(l_servo_results[AP10110] - l_servo_results[AP10010]) >> 1;
    o_hvals[1] = eo_dfe_round(l_servo_results[AP11010] - l_servo_results[AP10010]) >> 1;
    o_hvals[2] = eo_dfe_round(l_servo_results[AP11010] - l_servo_results[AP01010]) >> 1;
 ...
 * @return void
 */
void rx_eo_dfe_fast_apply(t_gcr_addr* i_tgt, int16_t i_loff_array[DAC_SIZE], int32_t i_hvals[3])
{
    int32_t  l_result   = 0;
    int32_t  l_index    = 0;
    ...
    for (; l_dac_addr <= DAC_END_ADDR; ++l_dac_addr, ++l_index)
    {
        ...
        // Convert Int to Signed Magnitude
        int32_t l_smag_dac = IntToSignedMag(l_result, DAC_WIDTH);

        // Write the final dac value
        put_ptr_fast(i_tgt, l_dac_addr, DAC_ENDBIT, l_smag_dac);
    }
 ...
 * @return uint32_t. pass_code if success, else error code.
 */
void  rx_eo_dfe_check_hvals(t_gcr_addr* i_tgt, int32_t i_hvals[3])
{
...
{
    const int32_t UNCERTAINTY_AVG  = 6;  // If a change is larger than this value, the result changes to the new value
    const int32_t UNCERTAINTY_HYST =
        3;  // If a change is larger than this value, the result changes to the average of the new and prev values, else it doesn't change
    ...
    if ( recal )
    {
        ...
        result = (delta <= UNCERTAINTY_HYST) ? i_prev :    // no change
                 (delta <= UNCERTAINTY_AVG ) ? (eo_dfe_round(i_new + i_prev) >> 1) :  // small change
                 i_new; // large change
    }
...
uint32_t rx_eo_dfe_full(t_gcr_addr* i_tgt, const t_bank i_bank, bool recal)
{
    ...
    for (; l_quad <= QUAD_WEST; l_quad += 8)
    {
        ...
        for (l_latch = L000; l_latch <= L111; ++l_latch)
        {
            ...
            l_dac_array[l_latch] = SignedMagToInt(get_ptr(i_tgt, (l_dac_addr + l_latch), DAC_STARTBIT, DAC_ENDBIT), DAC_WIDTH);
        }
        ...
        for (l_latch = L000; l_latch <= L111; ++l_latch)
        {
            ...
            int32_t l_new_val = eo_dfe_round((int32_t)l_ap_results[l_latch] + (int32_t)l_an_results[l_latch]) >> 1;
            ...
            {
                // Convert to dac format. 9 bit Signed Magnitude. first bit is polarity and next 8 is magnitude
                int32_t l_smag_dac = IntToSignedMag(l_new_val, DAC_WIDTH);
                SET_DFE_DEBUG(0x7017, l_smag_dac); // Update Latch Dac
                put_ptr_fast(i_tgt, (l_dac_addr + l_latch), DAC_ENDBIT, l_smag_dac);
            }
            else
            {
            ...
            {
                int check_dfe_max =  SignedMagToInt(mem_pg_field_get(rx_dfe_max_check), rx_dfe_max_check_width); //ppe pg
                int check_dfe_min =  SignedMagToInt(mem_pg_field_get(rx_dfe_min_check), rx_dfe_min_check_width); //ppe pg
