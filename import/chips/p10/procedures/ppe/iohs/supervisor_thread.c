...
//-------------|--------|-------------------------------------------------------
// vbr21011901 |vbr     | Added new debug log for thread lock error
// vbr21011900 |vbr     | Updated thread_lock check to avoid possibility of thread_loop_count incrementing exactly 64K times
// mbs20121700 |mbs     | HW556565: Skip recal not run when fw_stop_thread=1
// vbr20111800 |vbr     | HW552111: Added check of timebase stuck/rollover/random value
// mbs20111000 |mbs     | HW552062: Greatly extend recal not run timeout in P10 DD1 again, now to 50 seconds
...
#define THREAD_LOCK_CHECK_PERIOD_P10_DD1    PK_SECONDS((PkInterval)50)
#define RECAL_NOT_RUN_CHECK_PERIOD  PK_MILLISECONDS(500)
#define RECAL_NOT_RUN_CHECK_PERIOD_P10_DD1  PK_SECONDS((PkInterval)50)
...
void supervisor_thread(void* arg)
{
    ...
    while (true)
    {
        ...
        else     //!fw_stop_thread
        {
            ...
            /////////////////////////////////////////////////
            PkTimebase prev_time = current_time;
            current_time = pk_timebase_get();

            // HW552111: Check for timebase errors (stuck, rollover, random value set)
            ...
            {
                uint32_t uword_diff = (uint32_t)(current_time >> 32) - (uint32_t)(prev_time >> 32);
            ...
            if (current_time > thread_locked_check_time)
            {
                ...
                for (thread = 0; thread < io_threads; thread++)
                {
                    ...
                    // Get the new count
                    uint16_t loop_count = mem_regs_u16_base_get(pg_base_addr(ppe_thread_loop_count_addr, thread),
                                          ppe_thread_loop_count_mask, ppe_thread_loop_count_shift);

                    // Overwrite the count with 0 (thread will never set it to 0)
                    mem_regs_u16_base_put(pg_base_addr(ppe_thread_loop_count_addr, thread), ppe_thread_loop_count_mask,
                                          ppe_thread_loop_count_shift, 0);

                    // Set FIR if the count hasn't changed from 0
                    if (loop_count == 0)
                    {
#if IO_DEBUG_LEVEL >= 1
                        ADD_LOG(DEBUG_THREAD_LOCK_ERROR, mem_regs_u16_base_get(pg_base_addr(ppe_debug_state_addr, thread), ppe_debug_state_mask,
            ...
            if (current_time > recal_not_run_check_time)
            {
                ...
                for (thread = 0; thread < io_threads; thread++)
                {
                    ...
                    int stop_thread = fw_regs_u16_base_get(fw_base_addr(fw_stop_thread_addr, thread), fw_stop_thread_mask,
                                                           fw_stop_thread_shift);
