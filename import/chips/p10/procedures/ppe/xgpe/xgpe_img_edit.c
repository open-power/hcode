...
uint32_t updateXgpeImage( FILE* i_fpXgpeImg, uint32_t i_imgSize, uint8_t i_quiet );
uint32_t updateXpmrImage( FILE* i_fpXpmrHdrImg, uint32_t i_imgSize, uint8_t i_quiet );

int main(int narg, char* argv[])
{
    if (narg < 2)
    {
        printf("XGPE Img Edit Usage: %s <full path to image> <quiet>\n",
               argv[0]);
        return -1;
    }
    ...
    if( XPMR_HEADER_SIZE == size )
    {
        ...
        updateXpmrImage( fpImage, size, quiet );
    }
    ...
    if( XGPE_IMAGE  ==  imageType )
    {
        ...
        updateXgpeImage( fpImage, size, quiet );
    }
